...
Pick by `Route` (the `model` value). The **Tier** column says whether the model is always loaded (`resident`), is a gateway alias resolving onto the resident (`alias → resident`), or is pulled from codeserver on demand (`copy-back`, +1–3 min first-load — see [Weight offload](operations.md#weight-offload--codeserver-copy-back)). The resident's own 81 GB GGUF stays **local** (it is the default lane; a 13-minute pull per cold start is not acceptable there). Speed/mem are single-stream figures from prior sweeps (`bin/bench-models.py`; raw in `logs/`). **Settings for each model live in its launcher** (`bin/<launcher>`) and are explained in [Per-launcher details](#per-launcher-details).

| Route (`model`) | Tier | Use it for | tok/s | Max ctx | Peak mem | Engine | Launcher (`bin/`) |
|---|---|---|---:|---:|---:|---|---|
| **`deepseek-v4-flash-0731`** ⟵ aliases `laguna-s-2.1`, `qwen3.6-27b`, `qwen3.6-35b-a3b`, `nemotron-3-puzzle-75b`, `deepseek-v4-flash-ds4` | **resident** | **coding — default** | 20 | 131 k⁶ | ~86 GB | ds4 (Entrpi fork) | `launch-ds4-entrpi.sh` |
| `laguna-s-2.1` | alias → resident | coding default 07-22 → **08-01**; route kept, **weights deleted** 08-02⁷ | 43–54 | 256 k | ~101 GB | vLLM | `launch-vllm-laguna-s21-nvfp4-v2.sh` |
| `nemotron-3-puzzle-75b` | alias → resident | coding default 07-08 → 07-22; **weights deleted** 08-02⁷ | 28 | 131 k⁵ | ~65 GB | vLLM | `launch-vllm-nemotron-puzzle-75b-mtp.sh` |
| `qwen3.6-35b-a3b-vision` | **dark** | **vision** (images) — no headroom beside the resident | 56 | 131 k | ~34 GB | vLLM⁴ | `launch-vllm-qwen-fast.sh` |
...
- **`deepseek-v4-flash-ds4`** — the **older route name for the resident**, kept so existing clients keep working; it resolves to `deepseek-v4-flash-0731` at the gateway. It was the copy-back long-context *planner* lane from 2026-05-12 until the 08-01 promotion — same model, same engine family, now serving the default traffic instead of occasional planning work. Its 2026-04 → 08 engineering history (quant wall, engine forks, prefill/decode trade-offs) is the deep dive in [deepseek-v4-flash.md](deepseek-v4-flash.md).
...
> Measured on the *retired* dense-27B + 35B-MoE co-resident pair, but the lesson applies unchanged to any future co-resident pair — any two engines sharing GB10 contend for SM cycles. (Moot since 2026-07-22: one resident holds the box. Note the harder 2026-07-24 finding in §34 — next to a fully-resident vLLM engine, a second one can't even *load* — and that the current ~86 GB ds4 resident leaves ~35 GB, which is why an eval window means *parking* prod rather than co-loading beside it, §37.)
