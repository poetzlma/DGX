...
- **`resident`** (`swap: false, exclusive: false, persistent: true`) — a **single** always-on model. Since 2026-08-16 that is `qwen3.8-27b` (~90 GB of the 121 GB pool, pinned vLLM container on :9030, launcher `bin/launch-vllm-qwen38-prod.sh`); it replaced `deepseek-v4-flash-0731`, which held the slot 2026-08-01 → 08-16 and is now the cmd-blocked rollback. `ttl: 0` — no idle unload, plus an `on_startup` preload hook so it loads at service start. `healthCheckTimeout: 1800` covers its cold start. (Previous eras are preserved as config backups: `llama-swap.yaml.bak.20260801-prelaguna-swap` (solo laguna), `…20260722-prelaguna` (the `nemotron-3-puzzle-75b` + `qwen3.6-35b-a3b-vision` pair split `0.55`/`0.28`), and `llama-swap.full.bak` (full roster).)
...
Consequently **every entry in a key's allowlist must be a route that can actually be served.** As of **2026-08-17 that is exactly one name**: `qwen3.8-27b`. The gateway's `model_list` was pruned from 18 routes to 1, and all 12 API keys were re-scoped to that single model, so the catalog, the allowlists, and the loaded engine now agree. Anything else returns HTTP 400 (`could not find suitable inference handler`).

> **Re-adding a route takes three edits, not one:** an entry in `deployed.yaml`'s `model_list`, the model actually loadable in `config/llama-swap.yaml`, **and** the name added to whichever keys should reach it. Miss the third and the client gets an auth-shaped error that looks nothing like a routing problem.

Audited and cleaned 2026-08-13: six keys carried dead routes (`qwen3.6-35b-a3b-vision`, dark since 07-22; `deepseek-v4-flash` retired 06-27; `deepseek-v4-pro`, which never existed; and the whole Qwen3.5/gemma/minimax generation). Audit with:
