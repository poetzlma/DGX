#!/bin/bash
# Keep the deepseek-v4-flash-0731 resident warm: if the engine is down, poke llama-swap
...
LOG=/home/max/llm-stack/logs/ds4-keepalive.log

if curl -sf -m 5 http://127.0.0.1:9010/v1/models >/dev/null 2>&1; then
  exit 0
fi
...
curl -sf -m 1500 http://127.0.0.1:8080/v1/chat/completions \
  ...
  >/dev/null 2>&1
echo "$(date -Is) relaunch trigger finished (health: $(curl -sf -m 5 http://127.0.0.1:9010/v1/models >/dev/null 2>&1 && echo ok || echo STILL-DOWN))" >> "$LOG"
