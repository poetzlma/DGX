...
Two caveats the table can't carry. **Only the top row answers today** — `config/llama-swap.yaml` is in locked mode and exposes only the resident; every `copy-back` and `dark` row above will fail until `config/llama-swap.full.bak` is restored ([operations.md](docs/operations.md#groups-resident--experiments)). And **concurrency pays, up to a point**: the resident serves c=8 and aggregate was still climbing when measured — 26.7 → 93.7 → 167.3 tok/s at c=1/4/8 on code, 6.3× for a 17 % per-request cost ([§44](docs/decisions.md#44-speculative-decoding-bake-off-dspark-k4-is-the-best-draft-depth-sglangs-dgx-spark-config-will-not-run-here-2026-08-17)). That holds at ordinary prompt sizes; the KV pool fits only ~5.8 requests at the full 262 k, so eight simultaneous near-max prompts preempt. The old "flat past c=2" figure was measured at 120 k and is not a general law. The ds4-era "prefills serialize" rule ([§36](docs/decisions.md#36-ds4-is-the-coding-default-laguna-pulled-2026-08-01)) applies only if you roll back.
